# google-maps-mcp Makefile — Go training wheels for the Python-brained
#
...
LDFLAGS := -X github.com/shotah/google-maps-mcp/server.ServerVersion=$(VERSION)
CGO_ENABLED ?= 0
...
# Default coverage scope excludes the stdio CLI main.
...
cli: ## Build the MCP binary into ./bin/google-maps-mcp
	mkdir -p bin
	CGO_ENABLED=$(CGO_ENABLED) go build -ldflags "$(LDFLAGS)" -o bin/$(BINARY)$(EXE) .

install: ## Install binary into $$GOPATH/bin (or $$GOBIN) as google-maps-mcp
